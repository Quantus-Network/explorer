...
import { MULTISIG_CREATED_COLUMNS } from '@/components/common/table-columns/MULTISIG_CREATED_COLUMNS';
...
export const useMultisigCreatedTable = () => {
  ...
  const orderByObject = useOrderBy<MultisigCreatedSorts>(orderBy ?? '');
  ...
  const columns = useMemo(() => MULTISIG_CREATED_COLUMNS, []);
  const [rowCount, setRowCount] = useState<number>(
    data?.meta.aggregate.totalCount ?? 0
  );

  const table = useReactTable<MultisigCreated>({
