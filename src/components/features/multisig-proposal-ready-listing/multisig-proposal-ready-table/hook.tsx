...
import { MULTISIG_PROPOSAL_READY_COLUMNS } from '@/components/common/table-columns/MULTISIG_PROPOSAL_READY_COLUMNS';
...
export const useMultisigProposalReadyTable = () => {
  ...
  const orderByObject = useOrderBy<MultisigProposalReadySorts>(orderBy ?? '');
  ...
  const {
    ...
    data,
    error: fetchError
  } = api.multisigProposalReady.useGetAll({
  ...
  const columns = useMemo(() => MULTISIG_PROPOSAL_READY_COLUMNS, []);
  const [rowCount, setRowCount] = useState<number>(
    data?.meta.aggregate.totalCount ?? 0
  );

  const table = useReactTable<MultisigProposalReady>({
    data: data?.multisigProposalReadyEvents ?? [],
    columns,
