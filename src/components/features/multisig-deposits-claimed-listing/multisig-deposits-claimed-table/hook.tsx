...
import { MULTISIG_DEPOSITS_CLAIMED_COLUMNS } from '@/components/common/table-columns/MULTISIG_DEPOSITS_CLAIMED_COLUMNS';
...
export const useMultisigDepositsClaimedTable = () => {
  ...
  const orderByObject = useOrderBy<MultisigDepositsClaimedSorts>(orderBy ?? '');
  ...
  const {
    ...
    data,
    error: fetchError
  } = api.multisigDepositsClaimed.useGetAll({
  ...
  const columns = useMemo(() => MULTISIG_DEPOSITS_CLAIMED_COLUMNS, []);
  const [rowCount, setRowCount] = useState<number>(
    data?.meta.aggregate.totalCount ?? 0
  );

  const table = useReactTable<MultisigDepositsClaimed>({
    data: data?.multisigDepositsClaimedEvents ?? [],
    columns,
