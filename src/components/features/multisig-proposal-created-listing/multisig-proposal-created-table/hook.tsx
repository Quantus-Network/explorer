...
import { MULTISIG_PROPOSAL_CREATED_COLUMNS } from '@/components/common/table-columns/MULTISIG_PROPOSAL_CREATED_COLUMNS';
...
export const useMultisigProposalCreatedTable = () => {
  ...
  const orderByObject = useOrderBy<MultisigProposalCreatedSorts>(orderBy ?? '');
  ...
  const {
    ...
    data,
    error: fetchError
  } = api.multisigProposalCreated.useGetAll({
  ...
  const columns = useMemo(() => MULTISIG_PROPOSAL_CREATED_COLUMNS, []);
  const [rowCount, setRowCount] = useState<number>(
    data?.meta.aggregate.totalCount ?? 0
  );

  const table = useReactTable<MultisigProposalCreated>({
