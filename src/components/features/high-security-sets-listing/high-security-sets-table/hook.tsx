...
import { HIGH_SECURITY_SET_SORTS_LITERALS } from '@/constants/query-sorts';
...
export const useHighSecuritySetsTable = () => {
  ...
  const [sortBy, setSortBy] = useQueryState(
    'sortBy',
    parseAsStringLiteral(HIGH_SECURITY_SET_SORTS_LITERALS)
  );
  ...
  const handleChangeSorting: OnChangeFn<SortingState> = (sorting) => {
    if (typeof sorting === 'function') {
      ...
      if (newValue[0]?.id) {
        ...
        const newSortBy = `${key}_${order}` as HighSecuritySetSorts;
