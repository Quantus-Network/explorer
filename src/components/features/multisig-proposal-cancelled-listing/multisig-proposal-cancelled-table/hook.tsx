...
import { MULTISIG_PROPOSAL_CANCELLED_COLUMNS } from '@/components/common/table-columns/MULTISIG_PROPOSAL_CANCELLED_COLUMNS';
...
export const useMultisigProposalCancelledTable = () => {
  ...
  const orderByObject = useOrderBy<MultisigProposalCancelledSorts>(
    orderBy ?? ''
  );
  ...
  const {
    ...
    data,
    error: fetchError
  } = api.multisigProposalCancelled.useGetAll({
  ...
  const columns = useMemo(() => MULTISIG_PROPOSAL_CANCELLED_COLUMNS, []);
  const [rowCount, setRowCount] = useState<number>(
    data?.meta.aggregate.totalCount ?? 0
  );

  const table = useReactTable<MultisigProposalCancelled>({
    data: data?.multisigProposalCancelledEvents ?? [],
    columns,
