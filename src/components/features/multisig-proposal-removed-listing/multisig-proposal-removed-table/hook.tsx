...
import { MULTISIG_PROPOSAL_REMOVED_COLUMNS } from '@/components/common/table-columns/MULTISIG_PROPOSAL_REMOVED_COLUMNS';
...
export const useMultisigProposalRemovedTable = () => {
  ...
  const orderByObject = useOrderBy<MultisigProposalRemovedSorts>(orderBy ?? '');
  ...
  const {
    ...
    data,
    error: fetchError
  } = api.multisigProposalRemoved.useGetAll({
  ...
  const columns = useMemo(() => MULTISIG_PROPOSAL_REMOVED_COLUMNS, []);
  const [rowCount, setRowCount] = useState<number>(
    data?.meta.aggregate.totalCount ?? 0
  );

  const table = useReactTable<MultisigProposalRemoved>({
    data: data?.multisigProposalRemovedEvents ?? [],
    columns,
